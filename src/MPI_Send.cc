...
#define __MAKE_TYPE_BRANCH__(TMPI,T0,T1,T2,A1)                          \
  if (TSnd == TMPI and MyOv.T1)                                         \
    {                                                                   \
      ...
      T2 m = MyOv.A1;                                                   \
      OCTAVE_LOCAL_BUFFER(int, sridx, m.numel ());                   \
      ...
      for (octave_idx_type ix = 0; ix < m.cols () + 1; ix++)            \
        scidx[ix]= m.cidx (ix);                                         \
                                                                        \
      OCTAVE_LOCAL_BUFFER(T0, sdata, m.numel ());                    \
                                                                        \
      for (octave_idx_type ix = 0; ix < m.numel (); ix++)            \
        {                                                               \
          sdata[ix]= m.data (ix);                                       \
          sridx[ix]= m.ridx (ix);                                       \
        }                                                               \
      ...
      for (octave_idx_type i = 0; i < rankrec.numel (); i++)            \
        {                                                               \
          ...
          if (info != MPI_SUCCESS) return info;                         \
                                                                        \
          info = MPI_Send                                               \
            (sridx, m.numel (), MPI_INT, rankrec_ptr[i],             \
             tanktag[2], comm);                                         \
          if (info != MPI_SUCCESS) return info;                         \
                                                                        \
          info = MPI_Send                                               \
          ...
          if (info != MPI_SUCCESS) return info;                         \
                                                                        \
          info =  MPI_Send                                              \
            (sdata, m.numel (), TSnd, rankrec_ptr[i],                \
             tanktag[4], comm);                                         \
          if (info != MPI_SUCCESS) return info;                         \
        }                                                               \
    }                                                                   
  ...
  __MAKE_TYPE_BRANCH__(MPI_INT, bool, is_bool_type (), 
    ...
    { 
      ...
      OCTAVE_LOCAL_BUFFER(double, sdata1, m.numel ());
      OCTAVE_LOCAL_BUFFER(double, sdata2, m.numel ());
      ...
      for (octave_idx_type i = 0; i < rankrec.numel (); i++)
        {
          ...
          info =  MPI_Send
            (sridx, m.numel (), MPI_INT, rankrec_ptr[i], tanktag[2], comm);
          if (info != MPI_SUCCESS) return info;
          ...
          info =  MPI_Send 
            (sdata1, m.numel (), TSnd, rankrec_ptr[i], tanktag[4], comm);
          if (info != MPI_SUCCESS) return info;

          info =  MPI_Send
            (sdata2, m.numel (), TSnd, rankrec_ptr[i], tanktag[5], comm);
          if (info != MPI_SUCCESS) return info;
        }
    }
  return(info);
